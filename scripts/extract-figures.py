...
def drawing(svg: str) -> str:
    """Everything that decides what is DRAWN, with timing, ids and filter refs stripped.

    Used to pair a still with the animated figure it is the last frame of. The figures
    terminal's rule, 25 Jul: *the still is the animation's last frame, from the same
    source, so the two cannot drift apart.* fig-13 and fig-14 are ONE figure in two
    renderings, not two figures, and a registry that lists both invites someone to point
    an item at the still and get a picture that can never animate.

    ⭐ DERIVED, NEVER HAND LISTED. A typed list of which figure pairs with which goes
    stale the first time a figure is added and nobody notices."""
    ...
    s = re.sub(r'(filter|clip-path)="url\([^)]*\)"', "", s)

    def strip_timing(m):
        keep = [c for c in m.group(1).split() if not re.fullmatch(r"r\d+|m-[\w-]+", c)]
        return f'class="{" ".join(keep)}"' if keep else ""

    s = re.sub(r'class="([^"]*)"', strip_timing, s)
    s = re.sub(r"\s+", " ", s)
    s = re.sub(r"\s+/?>", lambda m: "/>" if m.group(0).endswith("/>") else ">", s)
    return s.strip()
...
def main() -> int:
    ...
    # not a finished state. Those simply find no partner here, which is correct.
    animated = {n: f for n, f in figures.items() if f["animated"]}
    for name, fig in figures.items():
        if fig["animated"]:
            continue
        for aname, afig in animated.items():
            if afig["still"] is None and afig["_drawing"] == fig["_drawing"]:
                fig["stillOf"] = aname
                afig["still"] = name
                break
    ...
    body = [
        ...
        "  animated: boolean;",
        "  /**",
        "   * ⭐ SET ON A STILL that is an animated figure's last frame. The two are ONE",
        "   * figure in two renderings, so ask for the ANIMATED name: it already collapses",
        "   * to this under prefers-reduced-motion. Derived by comparing the drawings with",
        "   * timing stripped, never hand listed.",
        "   */",
        "  stillOf: string | null;",
        "  /** Set on an animated figure that has a still twin. ⚠️ null on the strips",
        "   *  (fig-24, fig-25): they pan along a track and have no finished state. */",
        "  still: string | null;",
        "  svg: string;",
        "};",
        "",
    ...
    for f in figures.values():
        pair = (f"still of {f['stillOf']}" if f["stillOf"]
                else f"still: {f['still']}" if f["still"]
                else "no still" if f["animated"] else "unpaired")
        print(f"  {f['id']:<10} {f['name'] or '-':<8} "
              f"{'animated' if f['animated'] else 'still':<9} {len(f['svg']):>6} chars  {pair}")
    return 0
