...
import type { FileRow, Row, Section } from "./EverythingClient";

/**
 * ⭐ THE ONE BUILDER OF THE LIBRARY, 26 Sep 2026. This code lived inside /course/everything's
 * page component until the resource centre gave the library its own front door at
 * /resources/library. Two pages now show the same folders and files, and the rule from the
 * page header (no second copy of any list) means one function builds it for both. Nothing in
 * here changed when it moved; the comments are the ones written on the days each rule was
 * earned, kept with the code they explain.
 */
export type ModuleRow = { n: number; title: string; when: string; has: boolean };
...
export function buildLibrary(): Library {
  ...
  return { sections, modules, hidden, shelfCount: SHELF_COUNT, lessonRows, fileRows };
}
